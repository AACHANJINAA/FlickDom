...
- [2026-08-01 17시]
  - `TurnBasedFlickPiece`의 플릭 입력을 다시 마우스 기준으로 돌려, 현재 말을 좌클릭으로 집고 드래그한 뒤 놓아서 발사하도록 되돌렸다.
  - 드래그 중 `Esc`로 현재 당기기 입력을 취소할 수 있게 하고, 취소 시 궤적 프리뷰가 바로 사라지도록 정리했다.
  - `MonkeySlingshotFlickPresenter`는 기본적으로 플릭 프레젠테이션을 비활성화해, 플릭 단계에서 원숭이 시점 강제 이동과 조준 카메라 포커스가 다시 개입하지 않도록 바꿨다.
  - 플릭 상태 카메라는 플레이어 추적 대신 보드 고정 뷰를 사용하도록 분리하고, `PlacementCameraController`에서 말 선택/플릭/물리 처리 단계에 약 60도 사선의 플릭 전용 시점을 적용하도록 조정했다.
  - 플릭 전용 카메라의 XZ 기준점을 `TokenMapGridView`가 아닌 `GridCellCandidateResolver`의 물리 보드 중심으로 바꿔, 플릭 씬 위를 정확히 바라보도록 보정했다.
